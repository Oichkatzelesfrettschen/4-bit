...
| conflict | 98-095A printed page 15; Intel 1975 Data Catalog, 4040 printed page 6-10 | The functional board description names `CPU RESET` negative true and requires eight full instruction cycles or 64 external clock periods. The 4040 catalog names the package `RESET` input logic-one active. | The sources can describe opposite sides of an untraced board inversion, but the present ledger does not prove that path. The model records both polarities and does not drive a historical reset waveform. |
| partial | 98-095A Figure 3-14, printed page 3-7; PDF 3, drawing 2000318 | The suggested clock diagram and the CPU-card drawing share the 5.185 MHz oscillator, 74161/9316 counter with low parallel inputs, 74H00/7404 conditioning, and MH0026 phase-driver topology. | The matching topology supports the divide-by-seven functional target. The complete A7 feedback equation and the physical phi1/phi2 timing budget remain untraced. |
| partial | 98-095A printed page 52; PDF 7, drawing 2000319; PDF 10, IN-28 input | A14 produces a high write command for `CMA EX` or for the coincidence of `PM`, `OUT`, and write-enable. A29-5-6 inverts it before the active-low memory-module `WRITE/READ` input. | This binds controller conditions and card-input polarity. A14 timing, A29 delay, IN-28 buffering, and the 2102 R/W pulse remain open. |
