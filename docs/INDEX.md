# Documentation Program (2026-04-30T00:00:00Z)

Objective: Production-grade, registry-driven documentation synchronized with code, covering SIMD
cluster execution, dual UI (CLI/TUI/GUI), and emulator architecture. No toy models; complete,
elegant docs.

MUST DOCUMENT
- Build/run (README)
- Architecture and decisions (ARCHITECTURE.md)
- API interfaces and usage (docs/API.md)
- Configuration and env variables (docs/DEPLOYMENT.md)
- Deploy and operate (docs/DEPLOYMENT.md)
- Breaking changes and migration (docs/CHANGELOG.md)
- Dependencies and requirements (README)
- Installation requirements (mcs4-emu/INSTALLATION.md)

SHOULD DOCUMENT
- Troubleshooting (docs/TROUBLESHOOTING.md)
- Performance tuning and optimization (ARCHITECTURE.md)
- Contributing (docs/CONTRIBUTING.md)
- Limitations and workarounds (ARCHITECTURE.md)
- Testing strategies and coverage (STATUS.md/README)
- Security considerations (docs/DEPLOYMENT.md)
- Audit log and validation status (docs/AUDIT.md)

NICE TO DOCUMENT
- History and rationale (ARCHITECTURE.md)
- Related projects and resources (ARCHITECTURE.md)
- Future plans and roadmap (docs/ROADMAP.md)
- Contributors and acknowledgments (README)
- Benchmarks and performance metrics (docs/CHANGELOG.md)

Information Architecture
- Modular docs; central index (docs/INDEX.md); metadata registry (docs/meta/registry.yaml)
- Registry scope: docs/-rooted files, repo-root files, and mcs4-emu/ status
  files. guide/ is the mdBook layer: chapters either transclude registry-
  covered docs via {{#include}} or are marked "(WIP stub)" in guide/SUMMARY.md
- Cross-links across README, ARCHITECTURE, ROADMAP, STATUS; ToC for long docs
- Automation: link_check.sh, md_lint.sh; CI builds docs, validates links

# Senior Documentation Architect Guidance (2026-01-07T06:33:37Z)

MUST DOCUMENT
- How to build/run (README)
- Architecture and decisions (ARCHITECTURE.md)
- API interfaces and usage (docs/API.md)
- Configuration options and environment variables (docs/DEPLOYMENT.md)
- Deployment and operations (docs/DEPLOYMENT.md)
- Breaking changes and migration (docs/CHANGELOG.md)
- Dependencies and requirements (README)

SHOULD DOCUMENT
- Common issues and troubleshooting (docs/TROUBLESHOOTING.md)
- Performance tuning and optimization (ARCHITECTURE.md)
- Contributing guidelines (docs/CONTRIBUTING.md)
- System limitations and workarounds (ARCHITECTURE.md)
- Testing strategies and coverage (STATUS.md/README)
- Security considerations (docs/DEPLOYMENT.md)

NICE TO DOCUMENT
- Historical context and rationale (ARCHITECTURE.md)
- Related projects and resources (ARCHITECTURE.md)
- Future plans and roadmap (docs/ROADMAP.md)
- Contributors and acknowledgments (README)
- Benchmarks and performance metrics (docs/CHANGELOG.md)

Information Architecture Pattern
- Modular docs with imports; central index; metadata registry (docs/meta/registry.yaml)
- Cross-links and discovery aids; ToC for long documents; indices and search-friendly headings

Metadata Registry (YAML)
- Schema with version and last_updated; programmatic discovery for index generation and validation

Quality Checklist
- Completeness, Accuracy, Discoverability, Maintainability
- Automated link checks, markdown lint, registry validation in CI

Metrics & Anti-Patterns
- Coverage, Freshness (90-day alerts), Link validity, Readability
- Avoid outdated, duplicated, scattered info; centralize and automate

## Registry Snapshot
<!-- DOCS_REGISTRY_START -->
- "README.md"
- "requirements.md"
- "claude.md"
- "ARCHITECTURE.md"
- "SCOPING_ASSESSMENT.md"
- "mcs4-emu/CLAUDE.md"
- "mcs4-emu/STATUS.md"
- "mcs4-emu/INSTALLATION.md"
- "mcs4-emu/requirements.md"
- "mcs4-emu/crates/mcs4-bus/REQUIREMENTS.md"
- "mcs4-emu/crates/mcs4-chips/REQUIREMENTS.md"
- "mcs4-emu/crates/mcs4-core/REQUIREMENTS.md"
- "mcs4-emu/crates/mcs4-fpga/REQUIREMENTS.md"
- "mcs4-emu/crates/mcs4-gui/REQUIREMENTS.md"
- "mcs4-emu/crates/mcs4-intellec/REQUIREMENTS.md"
- "mcs4-emu/crates/mcs4-periph/REQUIREMENTS.md"
- "mcs4-emu/crates/mcs4-system/REQUIREMENTS.md"
- "docs/API.md"
- "docs/DEPLOYMENT.md"
- "docs/CONTRIBUTING.md"
- "docs/TROUBLESHOOTING.md"
- "docs/CHANGELOG.md"
- "docs/ROADMAP.md"
- "docs/DEBT_ROADMAP.md"
- "docs/DEVELOPER_BUNDLE.md"
- "docs/BUILD_ENVIRONMENT.md"
- "docs/SECURITY.md"
- "docs/CAPABILITY_MATRIX.md"
- "docs/TIMING_PROVENANCE.md"
- "docs/meta/capabilities.json"
- "docs/repository-debt-callgraph-capture-roadmap.md"
- "docs/TODO.md"
- "docs/INDEX.md"
- "docs/DEVELOPMENT.md"
- "docs/QUALITY-CHECKLIST.md"
- "docs/AUDIT.md"
- "docs/TOOLING_AUDIT.md"
- "docs/CHIP_ARTIFACTS.md"
- "docs/CHIP_EXTRACTION.md"
- "docs/NETLIST_WORKFLOW.md"
- "docs/NETLIST_V1_SCHEMA.md"
- "docs/LAYER_ANNOTATIONS.md"
- "docs/MCS40_SYSTEM_INTEGRATION.md"
- "docs/ACCURACY_PROGRAM.md"
- "docs/CLAIMS_TO_TESTS.md"
- "docs/TRANSISTOR_SOLVER_ARCHITECTURE.md"
- "docs/4101_DESIGN.md"
- "docs/4289_DESIGN.md"
- "docs/OCR_RICING.md"
- "docs/PHASE_LOG.md"
- "docs/photomicrographs/README.md"
- "docs/emulators/README.md"
- "docs/evidence/README.md"
- "docs/evidence/callgraph-runtime-capture.md"
- "docs/evidence/gate-hdl-export-contract.md"
- "docs/evidence/fpga-board-clock-and-conformance-blockers.md"
- "docs/evidence/timing_parameters.json"
- "docs/evidence/DIAGRAM_EXTRACTION.md"
- "docs/evidence/audit_claims_backlog.md"
- "docs/evidence/PRIMARY_SOURCES_BACKLOG.md"
- "docs/evidence/OCR_SIGNAL_LABELS.md"
- "docs/evidence/ANCHOR_COVERAGE_V0.md"
- "docs/evidence/POWER_RAIL_EVIDENCE.md"
- "docs/evidence/photomicrograph_permissions.md"
- "docs/evidence/PROVENANCE_CHECKLIST.md"
- "docs/evidence/ocr_manifest.yaml"
- "docs/evidence/source_manifest.json"
- "docs/evidence/bibliography.bib"
- "docs/evidence/CITATION_GUIDE.md"
- "docs/evidence/url_reachability_audit.md"
- "docs/evidence/ocr_results.md"
- "docs/evidence/CLUSTERING_STRATEGY_V0.md"
- "docs/evidence/FINAL_EXECUTION_SUMMARY.md"
- "docs/evidence/FPGA_SYNTHESIS_WORKFLOW_V0.md"
- "docs/evidence/interconnect_parameters.md"
- "docs/evidence/LACUNAE_STATUS.md"
- "docs/evidence/NETLIST_PARAM_SENSITIVITY.md"
- "docs/evidence/noise_parameters.md"
- "docs/evidence/OCR_TRAINING_PIPELINE_V0.md"
- "docs/evidence/packaging_parasitics.md"
- "docs/evidence/PERIPHERAL_INTERFACES_V0.md"
- "docs/evidence/PHASE_0.5_1_COMPLETION_SUMMARY.md"
- "docs/evidence/PHASE_0.5_ADDENDUM_OCR_CACHE.md"
- "docs/evidence/PHASE_0.5_CACHE_IMPLEMENTATION.md"
- "docs/evidence/PHASE_0.5_COMPLETION.md"
- "docs/evidence/PHASE_1_COMPLETION_SUMMARY.md"
- "docs/evidence/PHASE_4_COMPLETION_SUMMARY.md"
- "docs/evidence/PHASE_5_COMPLETION_SUMMARY.md"
- "docs/evidence/PRIMARY_SOURCE_PINOUTS.md"
- "docs/evidence/process_parameters_v0.md"
- "docs/evidence/process_parameters_v1.md"
- "docs/evidence/process_params_audit.md"
- "docs/evidence/reliability_models.md"
- "docs/evidence/SIMD_CLUSTER_DESIGN_V0.md"
- "docs/evidence/VERILOG_EXPORT_DESIGN_V0.md"
- "docs/archive/SESSION_LOG.md"
- "docs/archive/UNIFIED_COSMIC_ROADMAP.md"
- "docs/archive/TOOLING_AUDIT_20260129.md"
- "docs/archive/PHASE_4_STATUS.md"
- "docs/archive/gemini.md"
- "docs/archive/PHASE_2_CHECKPOINT.md"
- "docs/archive/NEXT_STEPS.md"
- "docs/archive/PHASE_2_STATUS.md"
- "docs/archive/PHASE_2_DEBUG_NOTES.md"
- "docs/archive/PHASE_3_STATUS.md"
<!-- DOCS_REGISTRY_END -->
